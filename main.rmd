...
* **H0 (Null Hypothesis):** The classification model cannot accurately predict the AI Risk Category of job roles based on salary, skill demand score, job openings, experience level, and education level at a rate better than random chance.
...
The algorithmic hyperparameter for the optimal number of k-neighbors will be dynamically assigned based on the square root of the training dataset’s size (k = $\sqrt{n}$).
...
With the introduction of AI in the tech industry, certain job markets have been significantly affected, while others have not. The researchers are to test whether job-related factors can be used to determine the level of risk associated with that position. Given this, the researchers decided to use K-nearest Neighbors (KNN) to build a model that can classify the level of AI risk for a job using data such as salary, geographic location, etc. 

**Results**

The model achieved an accuracy of 86.84%, which is sufficient for our research. This indicates that the model correctly predicted the AI Risk Category for most of the test observations. The results also suggest that the selected market-related variables, including salary, skill demand score, and job openings, contain useful patterns for distinguishing among High Risk, Medium Risk, and Low Risk job categories.
